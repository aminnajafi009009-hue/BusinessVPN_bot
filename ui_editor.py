...
def _build_auto_catalog_impl():
    ...
    for item in sorted(_AUTO_CATALOG.values(), key=lambda x: len(x["template"]), reverse=True):
        ...
        try:
            _AUTO_PATTERNS.append((re.compile(r"^" + regex + r"$", re.S), item, placeholders))
        except re.error:
            pass
    _AUTO_READY = True
...
def apply_auto_text(text: str) -> str:
    ...
    for regex, item, count in _AUTO_PATTERNS:
        m = regex.match(text)
        if not m:
            continue
...
def categories_keyboard(mode: str = "text"):
    _ensure()
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=v, callback_data=f"ui_cat:{mode}:{k}")]
        for k, v in CATEGORIES.items()
    ] + [[InlineKeyboardButton(text="🔙 بازگشت", callback_data="admin_text_editor")]])
...
def auto_source_groups_keyboard(page: int = 0, page_size: int = 8):
    ...
    rows = [
        ...
        for sf, label, count in page_groups
    ]
    nav = []
    if page > 0:
...
def auto_messages_keyboard(source_file: str, page: int = 0, page_size: int = 6):
    ...
    rows = []
    for item in items:
        label = item["label"][:40] + ("…" if len(item["label"]) > 40 else "")
        rows.append([InlineKeyboardButton(text=f"📝 {label}", callback_data=f"ui_auto:{item['key']}")])
    nav = []
    if page > 0:
...
def text_screen_keyboard(key: str, page: int = 0):
    ...
    visible = variants[start:start + page_size]
    for i, item in enumerate(visible, start + 1):
        label = item["label"][:42] + ("…" if len(item["label"]) > 42 else "")
        rows.append([InlineKeyboardButton(text=f"🧩 متن واقعی {i}: {label}", callback_data=f"ui_edit_auto:{item['key']}")])
    nav = []
    if page > 0:
